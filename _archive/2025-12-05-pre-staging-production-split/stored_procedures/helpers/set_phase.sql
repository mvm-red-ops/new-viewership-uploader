CREATE OR REPLACE PROCEDURE upload_db.public.set_phase_dynamic(platform VARCHAR, phase_number FLOAT)
RETURNS STRING
LANGUAGE JAVASCRIPT
EXECUTE AS CALLER
AS
...
try {
    ...
    var sql_command = `UPDATE test_staging.public.${jsPlatform}_viewership SET phase = ? WHERE processed IS NULL`;
