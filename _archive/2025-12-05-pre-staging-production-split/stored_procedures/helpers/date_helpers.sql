...
CREATE OR REPLACE FUNCTION upload_db.public.get_full_date(date STRING)
  RETURNS STRING
  LANGUAGE SQL
AS 
$$  
...
END
$$;
-- GRANT USAGE ON  FUNCTION upload_db.public.get_full_date(STRING) TO ROLE WEB_APP;

--used for getting the mm/dd/yyyy date from the mm/dd/yy viewership week (necessary for using the date_trunc function )upload_db.public.
CREATE OR REPLACE FUNCTION  upload_db.public.get_full_date_from(date STRING)
  RETURNS STRING
  LANGUAGE SQL
  AS 
  $$  
...
CREATE OR REPLACE FUNCTION  upload_db.public.get_week_start(date_string STRING)
  RETURNS DATE
  LANGUAGE SQL
  AS 
  $$  
...
CREATE OR REPLACE FUNCTION  upload_db.public.get_quarter_from_mm_dd_yyyy(date_string STRING)
  RETURNS STRING
  LANGUAGE SQL
  AS 
  $$  
...
--gets year based on date value in date column 
  CREATE OR REPLACE FUNCTION  upload_db.public.get_year_from_mm_dd_yyyy(date_string STRING)
  RETURNS NUMBER
  LANGUAGE SQL
  AS 
  $$  
...
--gets month based on date value in date column 
  CREATE OR REPLACE FUNCTION  upload_db.public.get_month_from_mm_dd_yyyy(date_string STRING)
  RETURNS NUMBER
  LANGUAGE SQL
  AS 
  $$  
...
--get first of month aka year_month_day based on date value in date column 
  CREATE OR REPLACE FUNCTION  upload_db.public.get_first_of_month_from_mm_dd_yyyy(date_string STRING)
  RETURNS STRING
  LANGUAGE SQL
  AS 
  $$  
...
--gets day based on date value in date column 
  CREATE OR REPLACE FUNCTION  upload_db.public.get_day_from_mm_dd_yyyy(date_string STRING)
  RETURNS NUMBER
  LANGUAGE SQL
  AS 
  $$  
