GRANT USAGE ON PROCEDURE upload_db.public.normalize_data_in_staging_roku(STRING) TO ROLE web_app;

CREATE OR REPLACE PROCEDURE upload_db.public.normalize_data_in_staging_roku(filename STRING)
RETURNS STRING
LANGUAGE JAVASCRIPT
EXECUTE AS OWNER
AS
$$
    ...
    try {
        ...
        var procedures = [
            { name: "upload_db.public.set_channel_dynamic_platform", args: ['Roku'] },
            { name: "upload_db.public.set_deal_parent_amagi", args: [] },
            { name: "upload_db.public.set_territory_amagi", args: [] },
            { name: "upload_db.public.set_date_columns_dynamic", args: ['Roku'] },
            { name: "upload_db.public.set_hov_mov_dynamic", args: ['Roku'] },
            { name: "upload_db.public.validate_nulls", args: ['Roku'] },
            { name: "upload_db.public.set_phase_dynamic", args: ['Roku', 1] }
        ];
