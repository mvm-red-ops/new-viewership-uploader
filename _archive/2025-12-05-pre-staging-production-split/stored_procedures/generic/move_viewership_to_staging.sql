...
CREATE OR REPLACE PROCEDURE UPLOAD_DB.PUBLIC.MOVE_VIEWERSHIP_TO_STAGING(
    platform STRING,
    filename STRING
)
RETURNS STRING
    ...
    try {
        ...
        if (hasCustomSanitization && sanitizationProc) {
            ...
            var sanitizeResult = executeStoredProcedure(
                `upload_db.public.${sanitizationProc}`,
                [FILENAME]
            );
            ...
            var genericSanitizeResult = executeStoredProcedure(
                'upload_db.public.generic_sanitization',
                [PLATFORM, FILENAME]
            );
        ...
        // Step 3: Move data from upload_db to test_staging
        var moveResult = executeStoredProcedure(
            'upload_db.public.move_sanitized_data_to_staging_generic',
            [PLATFORM, FILENAME]
        );
        ...
        var phaseResult = executeStoredProcedure(
            'upload_db.public.set_phase_generic',
            [PLATFORM, '0', FILENAME]
        );
...
-- Grant permissions
GRANT USAGE ON PROCEDURE UPLOAD_DB.PUBLIC.MOVE_VIEWERSHIP_TO_STAGING(STRING, STRING) TO ROLE web_app;
