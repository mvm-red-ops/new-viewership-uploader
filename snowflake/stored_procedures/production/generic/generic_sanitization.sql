...
CREATE OR REPLACE PROCEDURE UPLOAD_DB_PROD.PUBLIC.GENERIC_SANITIZATION(
    platform STRING,
    filename STRING
)
RETURNS STRING
...
try {
    ...
    var updateQuery = `
        UPDATE UPLOAD_DB_PROD.public.platform_viewership
        SET
...
-- Grant permissions
GRANT USAGE ON PROCEDURE UPLOAD_DB_PROD.PUBLIC.GENERIC_SANITIZATION(STRING, STRING) TO ROLE web_app;
