-- Generic Normalize Data in Staging Wrapper
-- For Streamlit uploads - calls all normalization procedures including SET_REF_ID_FROM_PLATFORM_CONTENT_ID
-- This is the procedure Lambda should call for the Streamlit processing path

CREATE OR REPLACE PROCEDURE UPLOAD_DB_PROD.PUBLIC.NORMALIZE_DATA_IN_STAGING_GENERIC(
    platform STRING,
    filename STRING
)
RETURNS STRING
...
$$
    ...
    try {
        ...
        snowflake.execute({
            sqlText: `CALL UPLOAD_DB_PROD.PUBLIC.SET_DEAL_PARENT_GENERIC(?, ?)`,
            binds: [platformArg, filenameArg]
        });

        // Step 2: Set ref_id from platform_content_id where applicable
        // THIS IS THE CRITICAL MISSING STEP IN LAMBDA!
        console.log('Step 2: Setting ref_id from platform_content_id...');
        snowflake.execute({
            sqlText: `CALL UPLOAD_DB_PROD.PUBLIC.SET_REF_ID_FROM_PLATFORM_CONTENT_ID(?, ?)`,
            binds: [platformArg, filenameArg]
        });
        ...
        snowflake.execute({
            sqlText: `CALL UPLOAD_DB_PROD.PUBLIC.CALCULATE_VIEWERSHIP_METRICS(?, ?)`,
            binds: [platformArg, filenameArg]
        });
        ...
        snowflake.execute({
            sqlText: `CALL UPLOAD_DB_PROD.PUBLIC.SET_DATE_COLUMNS_DYNAMIC(?, ?)`,
            binds: [platformArg, filenameArg]
        });
        ...
        snowflake.execute({
            sqlText: `CALL UPLOAD_DB_PROD.PUBLIC.SET_PHASE_GENERIC(?, ?, ?)`,
        ...
        const errorMessage = "Error in normalize_data_in_staging_generic: " + err.message;

        // Log the error
        try {
            snowflake.execute({
                sqlText: "INSERT INTO UPLOAD_DB_PROD.PUBLIC.ERROR_LOG_TABLE (LOG_MESSAGE, PROCEDURE_NAME, PLATFORM) VALUES (?, ?, ?)",
                binds: [errorMessage, 'normalize_data_in_staging_generic', platformArg]
            });
        } catch (logErr) {
...
-- Grant permissions
GRANT USAGE ON PROCEDURE UPLOAD_DB_PROD.PUBLIC.NORMALIZE_DATA_IN_STAGING_GENERIC(STRING, STRING) TO ROLE web_app;
