...
-- Simply copies data from UPLOAD_DB_PROD.public.platform_viewership to NOSEY_PROD.public.platform_viewership

CREATE OR REPLACE PROCEDURE UPLOAD_DB_PROD.PUBLIC.MOVE_STREAMLIT_DATA_TO_STAGING(
    platform STRING,
    filename STRING
)
RETURNS STRING
...
try {
    ...
    sql_command = `
        SELECT COLUMN_NAME
        FROM NOSEY_PROD.INFORMATION_SCHEMA.COLUMNS
        WHERE TABLE_SCHEMA = 'PUBLIC'
          ...
          AND COLUMN_NAME IN (
              ...
              WHERE TABLE_SCHEMA = 'PUBLIC'
                AND TABLE_NAME = 'PLATFORM_VIEWERSHIP'
                AND TABLE_CATALOG = 'UPLOAD_DB_PROD'
          )
    ...
    if (columns.length === 0) {
        throw new Error("No matching columns found between upload_db and NOSEY_PROD");
    }
    ...
    sql_command = `
        DELETE FROM NOSEY_PROD.public.platform_viewership
    ...
    return `Data copied successfully. ${rowsInserted} rows moved from upload_db to NOSEY_PROD for ${PLATFORM} - ${FILENAME}. Phase set to 0 for ${rowsUpdated} rows.`;
...
-- Grant permissions
GRANT USAGE ON PROCEDURE UPLOAD_DB_PROD.PUBLIC.MOVE_STREAMLIT_DATA_TO_STAGING(STRING, STRING) TO ROLE web_app;
