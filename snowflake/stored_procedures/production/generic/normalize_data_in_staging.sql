...
$$
    ...
    try {
        ...
        if (hasCustomTerritory && territoryProc) {
            procedures.push({
                name: `UPLOAD_DB_PROD.public.${territoryProc}`,
                args: [FILENAME]
            });
        }
        ...
        if (hasCustomChannel && channelProc) {
            procedures.push({
                name: `UPLOAD_DB_PROD.public.${channelProc}`,
                args: [FILENAME]
            });
        }
        ...
        if (dealParentProc) {
            if (dealParentColumns) {
                ...
                procedures.push({
                    name: `UPLOAD_DB_PROD.public.${dealParentProc}`,
        ...
        if (hasCustomDate && dateProc) {
            procedures.push({
                name: `UPLOAD_DB_PROD.public.${dateProc}`,
        ...
        if (additionalNormalizers) {
            try {
                ...
                for (var i = 0; i < additionalProcs.length; i++) {
                    ...
                    procedures.push({
                        name: `UPLOAD_DB_PROD.public.${procName}`,
                        args: [PLATFORM, FILENAME]
                    });
                }
            } catch (parseErr) {
        ...
        procedures.push({
            name: "UPLOAD_DB_PROD.public.set_phase_generic",
