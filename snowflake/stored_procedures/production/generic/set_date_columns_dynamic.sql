...
AS '
    ...
    if (!hasDateColumn) {
        ...
        updateCommands.push(`
            UPDATE NOSEY_PROD.public.platform_viewership
            SET week = UPLOAD_DB_PROD.public.get_week_start(TO_DATE(year_month_day, ''YYYYMMDD''))
            WHERE platform = ''${platform}''
              AND filename = ''${filename}''
              AND processed IS NULL
        ...
        updateCommands.push(`
            UPDATE NOSEY_PROD.public.platform_viewership
            SET day = UPLOAD_DB_PROD.public.get_day_from_mm_dd_yyyy(TO_DATE(year_month_day, ''YYYYMMDD''))
            WHERE platform = ''${platform}''
              AND filename = ''${filename}''
              AND processed IS NULL
              AND year_month_day IS NOT NULL
        ...
        updateCommands.push(`UPDATE NOSEY_PROD.public.platform_viewership
         SET full_date = UPLOAD_DB_PROD.public.get_full_date(date)
         WHERE platform = ''${platform}''
           AND filename = ''${filename}''
           AND processed IS NULL`);
        ...
        updateCommands.push(`UPDATE NOSEY_PROD.public.platform_viewership
         SET week = UPLOAD_DB_PROD.public.get_week_start(UPLOAD_DB_PROD.public.get_full_date(date))
         WHERE platform = ''${platform}''
           AND filename = ''${filename}''
           AND processed IS NULL`);
        ...
        updateCommands.push(`UPDATE NOSEY_PROD.public.platform_viewership
         SET quarter = UPLOAD_DB_PROD.public.get_quarter_from_mm_dd_yyyy(UPLOAD_DB_PROD.public.get_full_date(date))
         WHERE platform = ''${platform}''
           AND filename = ''${filename}''
           AND processed IS NULL`);
        ...
        updateCommands.push(`UPDATE NOSEY_PROD.public.platform_viewership
         SET year = UPLOAD_DB_PROD.public.get_year_from_mm_dd_yyyy(UPLOAD_DB_PROD.public.get_full_date(date))
         WHERE platform = ''${platform}''
           AND filename = ''${filename}''
           AND processed IS NULL`);
        ...
        updateCommands.push(`UPDATE NOSEY_PROD.public.platform_viewership
         SET month = UPLOAD_DB_PROD.public.get_month_from_mm_dd_yyyy(UPLOAD_DB_PROD.public.get_full_date(date))
         WHERE platform = ''${platform}''
           AND filename = ''${filename}''
           AND processed IS NULL`);
        ...
        updateCommands.push(`UPDATE NOSEY_PROD.public.platform_viewership
         SET year_month_day = UPLOAD_DB_PROD.public.get_first_of_month_from_mm_dd_yyyy(UPLOAD_DB_PROD.public.get_full_date(date))
         WHERE platform = ''${platform}''
           AND filename = ''${filename}''
           AND processed IS NULL`);
        ...
        updateCommands.push(`UPDATE NOSEY_PROD.public.platform_viewership
         SET day = UPLOAD_DB_PROD.public.get_day_from_mm_dd_yyyy(UPLOAD_DB_PROD.public.get_full_date(date))
         WHERE platform = ''${platform}''
           AND filename = ''${filename}''
           AND processed IS NULL`);
    }
