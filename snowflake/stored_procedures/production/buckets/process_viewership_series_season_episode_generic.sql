...
try {
    ...
    FROM (
        ...
        JOIN METADATA_MASTER.public.series s ON (UPLOAD_DB_PROD.public.extract_primary_title(s.titles) = v.internal_series)
        JOIN METADATA_MASTER.public.episode e ON (s.id = e.series_id )
        JOIN METADATA_MASTER.public.metadata m ON (m.ref_id = e.ref_id)
    ...
    try {
        ...
        const series_only_fallback_sql = `
            ...
            FROM (
                ...
                JOIN METADATA_MASTER.public.episode e ON (e.ref_id = m.ref_id)
