...
try {
    ...
    FROM (
        ...
        JOIN UPLOAD_DB_PROD.PUBLIC.TEMP_${platformArg}_${bucketName}_BUCKET b ON (v.id = b.id)
        ...
        JOIN METADATA_MASTER.public.metadata m_title_check ON (
        ...
        WHERE v.platform = ''${platformArg}''
            ...
            AND LOWER(UPLOAD_DB_PROD.public.extract_primary_title(s.titles)) = LOWER(v.internal_series)
        ...
        WHERE v.platform = ''${platformArg}''
            ...
            AND NOT EXISTS (
                SELECT 1
                FROM METADATA_MASTER.public.metadata m_check
