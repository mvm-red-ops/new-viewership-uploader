CREATE OR REPLACE PROCEDURE "ANALYZE_AND_PROCESS_VIEWERSHIP_DATA_GENERIC"("PLATFORM" VARCHAR, "FILENAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE JAVASCRIPT
EXECUTE AS CALLER
AS '
    ...
    if (filenameArg) {
        try {
            const deleteResult = snowflake.execute({
                sqlText: `DELETE FROM METADATA_MASTER_CLEANED_STAGING.public.record_reprocessing_batch_logs WHERE filename = ''${filenameArg.replace(/''/g, "''''")}''`
            });
    ...
    try {
        ...
        if (finalUnmatchedCount > 0) {
            ...
            const logFinalUnmatchedSql = `
                INSERT INTO METADATA_MASTER_CLEANED_STAGING.public.record_reprocessing_batch_logs (
                    title,
                    viewership_id,
                    filename,
                    notes,
                ...
                WHERE v.platform = ''${platformArg}''
                  ...
                  AND NOT EXISTS (
                      SELECT 1
                      FROM METADATA_MASTER_CLEANED_STAGING.public.record_reprocessing_batch_logs l
