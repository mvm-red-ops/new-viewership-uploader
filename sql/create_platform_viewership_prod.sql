...
-- PRODUCTION VERSION
-- This creates the table in upload_db_prod database

-- Create sequence for ID column
CREATE SEQUENCE IF NOT EXISTS upload_db_prod.public.platform_viewership_id_seq
    START = 1
    INCREMENT = 1;

CREATE TABLE IF NOT EXISTS upload_db_prod.public.platform_viewership (
    -- Primary Key
    ID NUMBER DEFAULT upload_db_prod.public.platform_viewership_id_seq.NEXTVAL NOT NULL,
...
-- ALTER TABLE upload_db_prod.public.platform_viewership CLUSTER BY (DATE, PLATFORM);
...
-- ALTER TABLE upload_db_prod.public.platform_viewership ADD SEARCH OPTIMIZATION;
